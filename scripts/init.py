...
ADOPTED_INSTRUCTIONS_RELPATH = ".ai-context/09-adopted-instructions.md"
ADOPTED_BACKUP_ROOT_RELPATH = ".ai-context/adopted-entry-backups"
...
def _rel_display(path: Path, target: Path) -> str:
    return str(path.relative_to(target)).replace(os.sep, "/")


class AdoptSource(NamedTuple):
    relpath: str
    path: Path
    content: str


class AdoptRecord(NamedTuple):
    relpath: str
    source_path: Path
    backup_path: Path
    content: str
...
def find_adoptable_entries(target: Path) -> List[AdoptSource]:
    """Return existing AI entry files that --adopt can import and replace."""
    entries: List[AdoptSource] = []
    for relpath in ADOPTABLE_ENTRY_RELPATHS:
        path = target / _rel(relpath)
        if path.is_file():
            entries.append(AdoptSource(relpath, path, path.read_text(encoding="utf-8")))
    return entries


def _adopt_records(target: Path, sources: List[AdoptSource], timestamp: str) -> List[AdoptRecord]:
    records: List[AdoptRecord] = []
    backup_root = target / _rel(ADOPTED_BACKUP_ROOT_RELPATH) / timestamp
    for source in sources:
        records.append(
            AdoptRecord(
                source.relpath,
                source.path,
                backup_root / _rel(source.relpath),
                source.content,
            )
        )
    return records
...
def render_adopted_instructions(records: List[AdoptRecord], adopt_time: str, target: Path) -> str:
    parts = [
        "# 09 · Adopted Instructions",
        "",
        "<!-- Generated by ai-handoff-init --adopt. Keep old entry-file rules here as source material. -->",
        "",
        f"**Adopt time**: {adopt_time}",
        "",
        "## Conflict Priority",
        "",
        ...
        "2. Current generated entry files (`CLAUDE.md`, `AGENTS.md`, `.github/copilot-instructions.md`)",
        "3. `.ai-context/02-conventions.md`",
        "4. Adopted legacy instructions in this file",
        "",
        "## Imported Entry Files",
        "",
    ]
    for record in records:
        fence = _markdown_fence_for(record.content)
        parts.extend([
            f"### `{record.relpath}`",
            "",
            f"- Original path: `{record.relpath}`",
            f"- Backup path: `{_rel_display(record.backup_path, target)}`",
            "",
            f"{fence}markdown",
...
def render_adopt_manifest(records: List[AdoptRecord], adopt_time: str, target: Path) -> str:
    lines = [
        "# Adopted Entry Backups",
        "",
        f"Adopt time: {adopt_time}",
        f"Imported into: `{ADOPTED_INSTRUCTIONS_RELPATH}`",
        "",
        "| Source | Backup |",
        "| --- | --- |",
    ]
    for record in records:
        lines.append(
            f"| `{record.relpath}` | `{_rel_display(record.backup_path, target)}` |"
        )
    return "\n".join(lines) + "\n"


def _insert_readme_adopt_row(text: str) -> str:
    row = "| `09-adopted-instructions.md` | 静态 | --adopt 收编的旧 AI 入口文件原文 |"
    if row in text:
        return text
    return _insert_before_marker(text, "\n## 三条铁律", row)


def _update_current_state_for_adopt(text: str) -> str:
    additions = [
        "- 已收编已有 AI 助手入口文件,原文见 `.ai-context/09-adopted-instructions.md`。",
        "- 旧入口文件已移动到 `.ai-context/adopted-entry-backups/`。",
        "- 复核 `.ai-context/09-adopted-instructions.md` 中的旧规则,必要时整理进 `02-conventions.md` 或 `04-decisions.md`。",
    ]
    out = text
...
def _update_session_log_for_adopt(text: str, records: List[AdoptRecord]) -> str:
    imported = ", ".join(f"`{record.relpath}`" for record in records)
    old_done = (
        "**完成 / Done**: 初始化 `.ai-context/` 目录与三个入口文件"
        "(`CLAUDE.md` / `AGENTS.md` / `.github/copilot-instructions.md`),项目骨架已建立。"
    )
    new_done = (
        "**完成 / Done**: 初始化 `.ai-context/` 目录与三个入口文件,并收编已有 AI 入口文件"
        f"({imported});旧内容导入 `.ai-context/09-adopted-instructions.md`。"
    )
    old_changed = (
        "**改动文件 / Changed files**: `.ai-context/*`, `CLAUDE.md`, `AGENTS.md`, "
        "`.github/copilot-instructions.md`"
    )
    new_changed = (
        "**改动文件 / Changed files**: `.ai-context/*`, "
        "`.ai-context/09-adopted-instructions.md`, "
        "`.ai-context/adopted-entry-backups/*`, `CLAUDE.md`, `AGENTS.md`, "
        "`.github/copilot-instructions.md`"
    )
    old_validation = "**验证 / Validation**: 初始化脚本成功写入目标文件。"
    new_validation = "**验证 / Validation**: 初始化脚本成功写入目标文件,并将旧入口文件移动到备份目录。"
    out = text.replace(old_done, new_done)
    out = out.replace(old_changed, new_changed)
    out = out.replace(old_validation, new_validation)
    return out


def apply_adopt_context_updates(
    plan: List[Tuple[Path, str]],
    target: Path,
    records: List[AdoptRecord],
    adopt_time: str,
    ...
    for dest, content in plan:
        relpath = _rel_display(dest, target)
        if relpath == ".ai-context/README.md":
            content = _insert_readme_adopt_row(content)
        elif relpath == ".ai-context/05-current-state.md":
            content = _update_current_state_for_adopt(content)
        elif relpath == ".ai-context/06-session-log.md":
            content = _update_session_log_for_adopt(content, records)
        updated.append((dest, content))

    updated.append((
        target / _rel(ADOPTED_INSTRUCTIONS_RELPATH),
        render_adopted_instructions(records, adopt_time, target),
    ))
    backup_root = records[0].backup_path
    for _ in Path(records[0].relpath).parts:
        backup_root = backup_root.parent
    manifest_path = backup_root / "MANIFEST.md"
    updated.append((manifest_path, render_adopt_manifest(records, adopt_time, target)))
    return updated


def move_adopted_entries(records: List[AdoptRecord]) -> None:
    for record in records:
        record.backup_path.parent.mkdir(parents=True, exist_ok=True)
        shutil.move(str(record.source_path), str(record.backup_path))


def print_adopt_plan(
    target: Path,
    plan: List[Tuple[Path, str]],
    records: List[AdoptRecord],
    dry_run: bool,
) -> None:
    prefix = "[adopt dry-run]" if dry_run else "[adopt]"
    print(f"{prefix} would import existing AI entry files in: {target}" if dry_run else f"{prefix} importing existing AI entry files in: {target}")
    print_tree(target, plan)
    print("\n  adopted entry moves:")
    for record in records:
        print(f"    {_rel_display(record.source_path, target)} -> {_rel_display(record.backup_path, target)}")
    print(f"\n  imported legacy instructions: {ADOPTED_INSTRUCTIONS_RELPATH}")
...
Examples:
  ...
  python init.py --adopt --name todo-api ...  # import existing entry files,
...
""",
    ...
    p.add_argument("--adopt", action="store_true",
                   help="import existing AI entry files into .ai-context/, move "
                        "the originals to adopted-entry-backups/, and write "
                        "fresh generated entry files")
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.adopt and (args.force or args.merge or args.print_snippets):
        sys.exit("error: --adopt cannot be combined with --force, --merge, or --print-snippets")
    if args.print_snippets and (args.force or args.merge or args.adopt or args.dry_run):
        sys.exit("error: --print-snippets cannot be combined with --force, --merge, --adopt, or --dry-run")
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if args.adopt:
        adopt_sources = find_adoptable_entries(target)
        if ai_ctx_conflicts:
            print("error: .ai-context/ already has files; --adopt cannot proceed:",
                  file=sys.stderr)
            for c in ai_ctx_conflicts:
                print(f"  {c}", file=sys.stderr)
            print("       keep the existing .ai-context/ and use --doctor, --upgrade,",
                  file=sys.stderr)
            print("       or --print-snippets instead of adopting entry files.",
                  file=sys.stderr)
            return 2
        if adopt_sources:
            now = _dt.datetime.now()
            timestamp = now.strftime("%Y%m%d-%H%M%S")
            adopt_time = now.strftime("%Y-%m-%d %H:%M:%S")
            records = _adopt_records(target, adopt_sources, timestamp)
            plan = apply_adopt_context_updates(plan, target, records, adopt_time)
            if args.dry_run:
                print_adopt_plan(target, plan, records, dry_run=True)
                total = sum(len(c.encode("utf-8")) for _, c in plan)
                print(f"\n  total: {len(plan)} files, {total:,} bytes")
                return 0

            print_adopt_plan(target, plan, records, dry_run=False)
            move_adopted_entries(records)
            written = write_plan(plan, backup_existing=False)
            print(f"\nwrote {len(written)} files to {target}")
            print_next_steps(ctx, args.stage)
            return 0

        print(
            "info: --adopt found no existing AI entry files; continuing with normal init.",
            file=sys.stderr,
        )
